...
/**
 * 收录内容的校验与解析（正文、标题、tags、内容哈希）。
 *
 * 从 DocumentIntakeService 抽出：A-2 的 PUT 更新要做同一套校验与解析，
 * 两处各写一份必然漂移——尤其是哈希口径，一旦不一致，"改了正文但哈希算法
 * 不同"会让变更检测失效。抽出后新增/更新走同一条解析路径。
 *
 * 抛出的 DocumentIntakeService.Rejected 保持不变：它是既有的对外契约
 * （HTTP 层映射 400），移动异常类型只会扩大 diff 而不带来收益。
 */
record DocumentContent(String title, String content, String contentHash, List<String> tags) {
    ...
    /**
     * 上传的文件：校验扩展名与大小，标题降级到文件名（去扩展名）。
     *
     * @return 解析结果与规范化后的来源文件名（source_uri）
     */
    static Parsed fromUpload(String filename, byte[] bytes) {
        String name = sanitizeFilename(filename);
        int dot = name.lastIndexOf('.');
        String extension = dot < 0 ? "" : name.substring(dot + 1).toLowerCase(Locale.ROOT);
        if (!ALLOWED_EXTENSIONS.contains(extension)) {
            throw new DocumentIntakeService.Rejected("不支持的文件类型"
                    + (extension.isBlank() ? "" : " ." + extension) + "（仅支持 .md / .txt）");
        }
        String fallbackTitle = dot < 0 ? name : name.substring(0, dot);
        return new Parsed(parse(bytes, fallbackTitle), limited(name, MAX_SOURCE_URI_CODE_POINTS));
    }

    /**
     * 直接提交的正文（PUT 的 JSON 形态）：没有文件名，标题降级到调用方给的
     * 兜底值——更新时是文档现有标题，不会因为去掉 H1 就把标题变成"未命名文档"。
     */
    static DocumentContent fromText(String text, String fallbackTitle) {
        if (text == null) {
            throw new DocumentIntakeService.Rejected("缺少正文内容");
        }
        return parse(text.getBytes(StandardCharsets.UTF_8), fallbackTitle);
    }

    private static DocumentContent parse(byte[] bytes, String fallbackTitle) {
        if (bytes.length > MAX_CONTENT_BYTES) {
            throw new DocumentIntakeService.Rejected("文件 " + describe(bytes.length) + " 超过收录上限 "
                    + describe(MAX_CONTENT_BYTES) + "，请拆分后再上传");
        }
        String content = stripBom(decodeStrictUtf8(bytes));
        if (content.isBlank()) {
            throw new DocumentIntakeService.Rejected("文件内容为空");
        }
        FrontMatter frontMatter = FrontMatter.parse(content);
        String title = resolveTitle(frontMatter.title(), frontMatter.body(), fallbackTitle);
        return new DocumentContent(title, content, sha256Hex(normalize(content)), frontMatter.tags());
    }

    /**
     * 标题降级顺序（子 Issue A §一.1）：frontmatter.title → 正文首个 # 一级
     * 标题 → 调用方兜底值。H1 扫描取 body（剥掉 frontmatter 块后的正文）；
    ...
    private static String resolveTitle(String frontMatterTitle, String body, String fallback) {
        ...
        return limited(fallback == null || fallback.isBlank() ? "未命名文档" : fallback, MAX_TITLE_CODE_POINTS);
    }
    ...
    private static String decodeStrictUtf8(byte[] bytes) {
        try {
            ...
            throw new DocumentIntakeService.Rejected("文件内容不是有效的 UTF-8 文本");
        }
    }
    ...
    private static String sanitizeFilename(String filename) {
        if (filename == null || filename.isBlank()) {
            throw new DocumentIntakeService.Rejected("缺少文件名");
        }
    ...
    /** 上传解析结果：内容 + 来源文件名（后者只有上传形态才有）。 */
    record Parsed(DocumentContent content, String sourceUri) {}
}
